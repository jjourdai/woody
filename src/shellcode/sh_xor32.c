...
*/
	...
	sh->content[sh->len + 1] = 0x8d;
	sh->content[sh->len + 2] = 0x3d;
	sh->off_vmaddr_text_ptr[0] = sh->len + 3;
	sh->content[sh->len + 7] = 0xba;
	sh->content[sh->len + 8] = (key >> 24) & 0xFF;
	sh->content[sh->len + 9] = (key >> 16) & 0xFF;
	sh->content[sh->len + 10] = (key >> 8) & 0xFF;
	sh->content[sh->len + 11] = (key >> 0) & 0xFF;
	sh->content[sh->len + 12] = 0xbe;
	sh->off_vmaddr_text_len[0] = sh->len + 13;
	sh->len += 17;
